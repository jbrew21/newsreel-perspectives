...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # data/*.json is regenerated by the daily pipeline. Served bare, it
        # carries only Last-Modified, so browsers fall back to HEURISTIC
        # caching and can pin a profile to yesterday's stances indefinitely
        # (Aug 29 2026: a freshly deployed profile rendered stale ordering
        # from cache while the server had the new file). Force revalidation —
        # Last-Modified makes the check a cheap 304 when nothing changed.
        if path.startswith('/data/') and path.endswith('.json'):
            self._force_revalidate = True
        return super().do_GET()

    def end_headers(self):
        if getattr(self, '_force_revalidate', False):
            self.send_header('Cache-Control', 'no-cache, must-revalidate')
            self._force_revalidate = False
